...
function SummaryBuyTable({ productData, selectMonth }: Props) {
  ...
  const currentProducts = buyProductList?.slice(productsOffset, endOffset)
  const pageCount = buyProductList
    ? Math.ceil(buyProductList?.length / productsPerPage)
    : 0

  const handlePageClick = (e: { selected: number }) => {
    const newOffset = buyProductList
